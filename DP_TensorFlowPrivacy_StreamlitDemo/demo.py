...
st.image(
    image,
    caption="Perfect privacy is achieved when the two databases that only \
...
with st.echo():
    optimizer = DPKerasSGDOptimizer(
        l2_norm_clip=1, noise_multiplier=1, num_microbatches=1, learning_rate=0.1
    )
...
if st.button("Run models with the above configurations"):
    ...
    score_train_DP, score_valid_DP, score_test_DP, eps, weights_DP = train(
        noise_multiplier=noise_multiplier,
        l2_norm_clip=l2_norm_clip,
        batch_size=batch_size,
        microbatches=1,
    ...
    data = {
        "Training Accuracy": [score_train_vanilla[1], score_train_DP[1]],
...
index_to_be_removed = st.slider(
    "Select the index of the data to remove",
    min_value=0,
...
if st.button("Remove the data point selected"):
    ...
    st.write(
        "the shape of the adversarial training set is: ", x_train_adversarial.shape
    )
    st.text("")
...
if st.button("train the models"):
    ...
    (
        ...
        score_test_vanilla,
        _,
        weights_nonprivate_adv,
    ) = train(
        noise_multiplier=noise_multiplier,
        l2_norm_clip=l2_norm_clip,
        batch_size=batch_size,
        microbatches=1,
        x_train=x_train_adversarial,
        y_train=y_train_adversarial,
        x_valid=x_valid,
        y_valid=y_valid,
        x_test=x_test,
        y_test=y_test,
    ...
    score_train_vanilla, score_valid_vanilla, score_test_vanilla, _, weights = train(
        noise_multiplier=noise_multiplier,
        l2_norm_clip=l2_norm_clip,
        batch_size=batch_size,
        microbatches=1,
    ...
    for index, value in enumerate(weights_nonprivate_adv):
        ...
        sums.append(
            np.sum(
                np.abs(weights_nonprivate_adv[index])
                + np.abs(weights_nonprivate[index])
            )
        )
    overall_sum_model_weights = np.sum(sums)
    # flatten the list of diffs between model parameters into one long list
    ...
    st.write(
        "The weights of the models trained with DP-SGD changed by: ",
